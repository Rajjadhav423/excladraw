...
const SelectionHandles = memo(function SelectionHandles({ shape, zoom, onResizeStart }: Props) {
  ...
  // Read CSS variable for accent color
  const accent = typeof window !== "undefined"
    ? getComputedStyle(document.documentElement).getPropertyValue("--accent").trim() || "#0C66E4"
    : "#0C66E4";
  const panelBg = typeof window !== "undefined"
    ? getComputedStyle(document.documentElement).getPropertyValue("--bg-panel").trim() || "white"
    : "white";
